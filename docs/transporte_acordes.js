/*!
 * ani-ann Chord Transposer plugin 2025
 */
(function($) {
  ...
  $.fn.transpose = function(options) {
    ...
    var updateChordDiagrams = function() {
        var chords = new Set($("span.c").map(function() {
            return $(this).text();
        }).get());

        var container = $("#chord-diagrams");
        container.empty();

        chords.forEach(function(chord) {
            var svgUrl = "https://archive.org/download/plugin-funcional_202501/" + chord.replace("/", "") + ".svg";
            container.append('<object type="image/svg+xml" data="' + svgUrl + '" class="styled-svg"></object>');
        });
    };
    ...
    return $(this).each(function() {
      ...
      updateChordDiagrams(); // Initial load of chord diagrams
    });
  };
